<?php 
	...
	if ( !empty($_POST)) {
		...
		if ($valid) {
			...
			$sql = "UPDATE assistors  set aName = ?, aAddress = ?, aMunicipality =?, aState =?, aZip =?, WHERE VANID = ?";
			$q = $pdo->prepare($sql);
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * FROM assistors where aName = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($name));
		$data = $q->fetch(PDO::FETCH_ASSOC);
		$name = $data['aName'];
		$address = $data['aAddress'];
		$city = $data['aMunicipality'];
		$state = $data['aState'];
                $zip = $data['aZip'];
		Database::disconnect();
	}
  ...
  function convertCharStr2jEsc(str, cstyle)
  {
    ...
    <div class="container">
    			...
    			<div class="span10 offset1">
    				<div class="row">
		    			<h3>Update Assistor</h3>
		    		</div>
    		
	    			<form class="form-horizontal" action="update.php?id=<?php echo $name?>" method="post">
